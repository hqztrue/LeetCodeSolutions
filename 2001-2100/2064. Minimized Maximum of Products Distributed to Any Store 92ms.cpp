...
public:
	...
	int minimizedMaximum(int t, vector<int>& A) {
		n=A.size(); a=&A[0]; m=min(n,(int)(pow(n*log2(N),2./3)*0.1+10)); s=0;
		for (int i=0;i<n;++i)s+=a[i];
		if (s<=t)return 1;
		for (int i=0;i<m;++i)swap(a[i],a[i+_rand()%(n-i)]);  //random_shuffle(a,a+n);
		int l=1,r=min(inf,(int)ceil(1.*s/(max(t-n,1)))),delta=m*log2(N)*0.05+50;
		while (l<r){
			int mid=(l+r)>>1;
