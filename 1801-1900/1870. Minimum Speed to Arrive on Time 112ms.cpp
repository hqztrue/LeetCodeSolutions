...
public:
	...
	int smallestDivisor(vector<int>& _A, int t) {
		vector<int> A=_A;
		...
		int l=1,r=min((ll)inf,(ll)ceil(1.*s/(max(t-n,1)))),delta=m*log2(N)*0.2+100;
		while (l<r){
			int mid=(l+r)>>1;
	...
	int minSpeedOnTime(vector<int>& A, double _t) {
		int ans=smallestDivisor(A,ceil(_t));
		if (ans==-1)return -1;
		n=A.size(); a=&A[0]; int res=n-1;
		for (int i=0;i<n-1;++i)res+=(a[i]-1)/ans;
		if (res+(double)a[n-1]/ans<=_t)return ans;
		res=ceil(a[n-1]/(_t-int(_t))-eps);
		for (int i=0;i<n-1;++i){
			int cur=ceil(1.*a[i]/ans);
			if (cur>1)res=min(res,(int)ceil(1.*a[i]/(cur-1)));
		}
		return res;
	}
};
